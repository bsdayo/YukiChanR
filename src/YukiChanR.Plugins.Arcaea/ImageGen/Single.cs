...
public partial class ArcaeaImageGenerator
{
    public async Task<byte[]> GenerateSingleAsync(ArcaeaRecord record, ArcaeaUserPreferences pref)
    {
...

        {
            ...
            canvas.DrawText(record.Score.ToDisplayScore(), 450, 944, textPaint);
        }
...

        {
            ...
            canvas.DrawLimitedText($"{days}d", 691, 1337, textPaint, 100);
        }
...

        {
            ...
            canvas.DrawText(record.Potential.ToString("0.0000"), 166, 1337, textPaint);
...

        {
            ...
            canvas.DrawText(_localizer["Remark"], 450, 1477, textPaint);
        }
